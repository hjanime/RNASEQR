...
'''
RNASEQRp is use pipe to connect different counterparts of RNASEQR. Thus it would be faster than the version of RNASEQR using disks as joints.


Abner Huang, 2010/11/11, at ISB, <abnercyh@cs.nthu.edu.tw>
'''
...
if __name__ == '__main__':
	description='''RNASEQR is developed to analyze the RNA-seq data. User can set the parameters of RNASEQR as arguments directly, if the configuration file is not given. If the augments and configuration file both are given, then the values of arguments will replace the values of parameters in configuration file.'''
	parser = argparse.ArgumentParser(description=description)
	parser.add_argument('configuration', metavar='setting.config', type=str, nargs='?',help='configuration file. If the file is given, the command-line parameters will replace parameters in the file enforcely.',default='')
	...
	parser.add_argument('-s', dest='phred_type', metavar='TYPE', type=str, help='phred_type: "Sanger","Solexa","Illumina1.3","Illumina1.5+"',choices=["Sanger","Solexa","Illumina1.3","Illumina1.5+"],default='')
	...
	parser.add_argument('--version', action='version', version='RNASEQR 0.6, 2011-08-11. Code developer/maintainer: Abner Huang, <abnercyh@cs.nthu.edu.tw>')
	...
	colorspace,paired_end_gap,debug = -1,-1,-1
	...
	print "\tPhred: %s\n\tAnnotation: %s [%s,%s]\n\tGenome: [%s]" % (phred_type,annotation_type,annotation_file,annotation_index_prefix,genome_index_prefix)
	...
	if paired_end_input==True:
		...
		instrcution='''%s  -v  %d  -p  %d %s -k 40 -m 40 --best --strata -S --sam-nohead -I %d -X %d --un phase1.pe.unmapped.fastq --max /dev/null %s %s -S --sam-nohead  %s  -1 %s -2 %s ''' % (aligner_path,align_mismatch,number_of_thread,mate_orientation,paired_end_gap[0],paired_end_gap[1],phred_type,colorspace,annotation_index_prefix,input_fastq[0],input_fastq[1])
		print instrcution
		prev_time=time()
		bundle_pe.verify_for_paired_end_reads(instrcution,"phase1.unique.sam",ENST_structures)
	else:
		instrcution='''%s  -v  %d  -p  %d  -k 40 -m 40 -q --un phase1.unmapped.fastq --max /dev/null --best --strata  %s %s -S --sam-nohead  %s  %s  ''' % (aligner_path,align_mismatch,number_of_thread,phred_type,colorspace,annotation_index_prefix,input_fastq)
		prev_time=time()
	...
	instrcution='''%s  -v  %d  -p  %d  -k 40 -m 40 -q --un /dev/null --max /dev/null --best --strata  %s  -S --sam-nohead   %s   ''' % (aligner_path,seg_align_mismatch,number_of_thread,annotation_index_prefix,"phase2.anchors.fastq")
	prev_time=time()
	#PEEDUP.verification_for_bowtie(instrcution,"stage2.T.unique.sam",ENST_structures)
	verification_for_bowtie_PIPE(instrcution,"stage2.T.unique.sam",ENST_structures)
	
	instrcution='''%s  -v  %d  -p  %d  -k 1 -m 1 -q --un /dev/null --max /dev/null --best --strata  %s  -S --sam-nohead   %s  > stage2.G.sam ''' % (aligner_path,seg_align_mismatch,number_of_thread,genome_index_prefix,"phase2.anchors.fastq")
	retcode = subprocess.call(instrcution,shell=True)
	if retcode < 0:
	...
	instrcution=' python '+RNASEQR_path+'/extract_sequence_for_stage2.py phase2.unmapped.fastq stage2.T.unique.sam stage2.G.unique.sam stage2.report -i '+genome_index_prefix_for_local_aligner+' -p '+local_aligner_path
	print instrcution
	prev_time=time()
	retcode = subprocess.call(instrcution,shell=True)
	if retcode < 0:
